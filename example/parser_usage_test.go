...
import (
	...
	"github.com/daiguadaidai/sql_parser/parser"
	"testing"
)
...
`
	...
	for _, stmtNode := range stmtNodes {
		// createStmt := stmtNode.(*ast.CreateTableStmt)
...
func TestParser_Sep(t *testing.T) {
	sql := `
alter table employees add column age1 int not null default 0 comment 'aaal';
`
...
func TestParser_NormalParse(t *testing.T) {
	sqlStr := `
SELECT /*! a hint */ a+1,
    IFNULL(a, 1)
FROM t
WHERE id = 1
    AND time = YEAR("2015-01-01");
`

	fmt.Println(parser.DigestHash(sqlStr))
	sqlTMP := parser.Normalize(sqlStr)
	fmt.Println(sqlTMP)
}
